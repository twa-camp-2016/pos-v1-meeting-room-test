'use strict';
let {formatTags, countBarcodes, getCartItems, getPromotions, getTotalPrices, getReceipt, printReceipt} = require('../main/main');
...
describe('pos', () => {
  ...
  it('#3| build cart items', ()=> {
    ...
    const cartItems = getCartItems(countedBarcodes, allItems);
    const expected = [
  ...
  it('#4| build promoted items', ()=> {
    ...
    const promotedItems = getPromotions(cartItems, promotions);
    const expected = [
      {
        barcode: 'ITEM000000', name: '可口可乐', unit: '瓶', price: 3.00, count: 3,
        payPrice: 6, saved: 3
  ...
  it('#5| calculate total prices', ()=> {
    ...
    const totalPrices = getTotalPrices(promotedItems);
    const expected = {
      totalPayPrice: 28.75,
      totalSaved: 3
    };
  ...
  it('#6| build receipt', ()=> {
    ...
    const receipt = getReceipt(promotedItems, totalPrices);
    const expected = {
      receiptItems: [
        {
          name: '可口可乐',
...
名称:雪碧,数量:5,单价:3.00(元),小计:12.00(元)
名称:荔枝,数量:2.5,单价:15.00(元),小计:37.50(元)
名称:方便面,数量:3,单价:4.50(元),小计:9.00(元)
----------------------
总计:58.50(元)
节省:7.50(元)
**********************`.trim();
